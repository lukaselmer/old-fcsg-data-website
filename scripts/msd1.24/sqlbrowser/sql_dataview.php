...
if ($showtables == 0)
{
	...
	echo '<a href="sql.php?db=' . $databases['db_actual'] . '&amp;dbid=' . $dbid . '&amp;tablename=' . $tablename . '&amp;context=2">' . $lang['sql_edit_tablestructure'] . '</a>';
}
else
{
...
echo '</p><p class="tablename">' . ( $tn != '' ? $lang['table'] . ' <strong>`' . $databases['db_actual'] . '`.`' . $tn . '`</strong><br>' : '' );
		...
		for ($i=0; $i < $numrows; $i++)
		{
			...
			foreach ($sortkey as $rowkey=>$rowval)
			{
				...
				if ($erste_spalte == 1)
				{
					...
					$p='sql.php?sql_statement=' . urlencode($sql['sql_statement']) . '&amp;db=' . $databases['db_actual'] . '&amp;tablename=' . $tablename . '&amp;dbid=' . $dbid . '&amp;limitstart=' . $limitstart . '&amp;order=' . $order . '&amp;orderdir=' . $orderdir . '&amp;tdc=' . $tdcompact;
					if ($key == -1)
					{
