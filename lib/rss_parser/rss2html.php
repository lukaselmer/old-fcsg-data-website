...
$XMLfilename = "http://feeds.feedburner.com/fcsg?format=xml";
...
$TEMPLATEfilename = "fcsg-template.html";
    ...
    if ($parseResult == 0) {
      ...
      if (!function_exists("FeedForALL_rss2html_replaceInItem")) {
        Function FeedForALL_rss2html_replaceInItem($source, $currentItem) {
          ...
          $pattern = '/<img[^>]+>/is';
          $descNoImg = preg_replace($pattern, '', $currentItem->description);

          $item = FeedForAll_rss2html_str_replace("~~~ItemDescriptionNoImg~~~", FeedForAll_rss2html_limitLength($descNoImg, $limitItemDescriptionLength), $item);
